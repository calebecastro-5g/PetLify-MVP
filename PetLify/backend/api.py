...
def update_vaccine_record(record_id):
    ...
    if data.get('valid_until'):
        try:
            record.valid_until = parse_datetime(data['valid_until'], 'Data de validade')
        except ValueError as exc:
            return jsonify({'error': str(exc)}), 400
...
def list_payments():
    user = current_user()
    payments = Payment.query.filter_by(store_id=user.store_id).order_by(Payment.created_at.desc()).all()
...
def list_employees():
    user = current_user()
    employees = User.query.filter_by(store_id=user.store_id).filter(User.role != Role.CLIENT).order_by(User.name.asc()).all()
