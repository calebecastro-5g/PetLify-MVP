...
const initialEmployeeForm = { name: '', email: '', phone: '', password: 'Dev@123456' };
...
function OwnerDashboard() {
  ...
  const handleCreateEmployee = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await apiFetch('/api/employees', {
        method: 'POST',
        body: JSON.stringify({ ...employeeForm, role: 'funcionario' }),
      });
  ...
  const handleDeleteEmployee = async (employee: Employee) => {
    if (!window.confirm(`Deseja excluir definitivamente o funcionário ${employee.name}?`)) return;
    setUpdatingEmployeeId(employee.id);
    setError('');
    setFeedback('');
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        setError(data.error || 'Não foi possível excluir o funcionário.');
        return;
      }
      setFeedback('Funcionário excluído com sucesso.');
      await loadData();
    } catch (err) {
      setError('Falha de conexão ao excluir funcionário.');
    } finally {
      setUpdatingEmployeeId(null);
    }
  };
        ...
        <>
          ...
          <Section id="equipe">
            ...
            <FormCard onSubmit={handleCreateEmployee}>
              ...
              <ReadOnlyRole><span>Perfil</span><strong>Funcionário</strong></ReadOnlyRole>
              <ActionButton type="submit" disabled={savingEmployee}>{savingEmployee ? <Spinner size="18px" /> : 'Cadastrar funcionário'}</ActionButton>
            </FormCard>
              ...
              {employees.map((employee) => (
                <ListCard key={employee.id}>
                  ...
                  <ButtonGroup>
                    ...
                    {employee.role === 'funcionario' && <DangerButton type="button" onClick={() => handleDeleteEmployee(employee)} disabled={updatingEmployeeId === employee.id}>Excluir</DangerButton>}
                  </ButtonGroup>
                </ListCard>
              ))}
            </List>
...
const DangerButton = styled.button`min-height:44px;border:0;border-radius:16px;padding:0 18px;background:#C93C58;color:#fff;font-weight:900;display:inline-flex;align-items:center;justify-content:center;gap:8px;&:disabled{opacity:.55;}`;
const ButtonGroup = styled.div`display:flex;gap:10px;flex-wrap:wrap;justify-content:flex-end;`;
const ReadOnlyRole = styled.div`display:grid;gap:8px;font-weight:800;strong{min-height:46px;border:1px solid #DDEAF3;border-radius:16px;padding:0 14px;background:#fff;color:#17324D;display:flex;align-items:center;}`;
