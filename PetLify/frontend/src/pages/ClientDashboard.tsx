...
const initialPetForm = { name: '', species: 'Cão', breed: 'SRD', size: 'Pequeno', age: '1', plan: 'Sem plano' };
const initialAppointmentForm = { petId: '', service: platformServices[0].name, scheduledAt: '', notes: '' };
...
function ClientDashboard() {
  ...
  const selectedService = useMemo(() => platformServices.find((service) => service.name === appointmentForm.service) || platformServices[0], [appointmentForm.service]);
  ...
  const startEditAppointment = (appointment: Appointment) => {
    ...
    const iso = new Date(appointment.scheduled_at).toISOString();
    setAppointmentForm({
      petId: String(appointment.pet_id),
      service: appointment.service,
      scheduledAt: iso.slice(0, 16),
  ...
  return (
    <DashboardShell
      ...
      subtitle="Cadastre e gerencie seus pets, acompanhe vacinas, pague planos e compre serviços avulsos somente dentro do fluxo de agendamento."
      ...
      {loading ? (
        ...
        <>
          ...
          <Section id="pets">
            ...
            {showPetForm && (
              <FormCard onSubmit={handleSavePet}>
                ...
                <Field><span>Raça</span><input value={petForm.breed} onChange={(event) => setPetForm((prev) => ({ ...prev, breed: event.target.value }))} required /></Field>
          ...
          <Section id="agenda">
            ...
            {showAppointmentForm && (
              <FormCard onSubmit={handleSaveAppointment}>
                ...
                <Field><span>Serviço avulso</span><select value={appointmentForm.service} onChange={(event) => setAppointmentForm((prev) => ({ ...prev, service: event.target.value }))}>{platformServices.map((service) => <option key={service.id} value={service.name}>{service.displayName}</option>)}</select></Field>
                <Field><span>Valor</span><ReadOnlyValue>{formatCurrency(selectedService.amount)}</ReadOnlyValue></Field>
                ...
                <HelperBox className="full"><strong>Fluxo ajustado conforme QA:</strong> o serviço avulso é contratado durante o agendamento e o pagamento é concluído na próxima etapa.</HelperBox>
                <WideButton type="submit" disabled={savingAppointment}>{savingAppointment ? <Spinner size="18px" /> : editingAppointmentId ? 'Salvar alterações' : 'Agendar e ir para pagamento'}</WideButton>
              </FormCard>
            )}

            {appointments.length === 0 ? (
              <EmptyState><strong>Nenhum agendamento encontrado.</strong><span>Após cadastrar um pet, clique em Novo Agendamento para marcar um serviço.</span></EmptyState>
            ) : (
              <List>
          ...
          <Section id="pagamentos">
            <SectionHeader>
              <div>
                <Eyebrow>Histórico financeiro</Eyebrow>
                <h2>Meus pagamentos</h2>
              </div>
            </SectionHeader>
            {payments.length === 0 ? (
              <EmptyState><strong>Nenhum pagamento registrado.</strong><span>Após contratar um plano ou concluir um agendamento, ele aparecerá aqui.</span></EmptyState>
            ) : (
              <List>
                {payments.map((payment) => (
                  <ListCard key={payment.id}>
                    <div>
                      <strong>{payment.item_name || 'Pagamento'}</strong>
                      <span>{payment.item_type || 'Item'} • {payment.method}</span>
          ...
          <Section id="planos">
            ...
            <HelperBox><strong>Importante:</strong> serviços avulsos não são comprados diretamente nesta tela. Eles são contratados dentro do fluxo de agendamento.</HelperBox>
          </Section>
        </>
      )}
    </DashboardShell>
...
const HelperBox = styled.div`padding:14px 16px;border-radius:18px;background:#FFF6CC;color:#17324D;line-height:1.5;`;
...
const SubsectionTitle = styled.h3`margin:4px 0 12px;color:#17324D;`;
const PriceCard = styled.article`padding:18px;border-radius:22px;background:#F8FCFF;border:1px solid #DDEAF3;display:grid;gap:6px;span{color:#64748B;}small{font-size:1.05rem;font-weight:900;color:#256D85;}`;
