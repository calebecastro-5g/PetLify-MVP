...
function ProfileEditor({ description }: ProfileEditorProps) {
  ...
  const handleDeleteAccount = async () => {
    const confirmed = window.confirm('Deseja realmente excluir sua conta? Essa ação remove o perfil do MVP e você sairá do sistema.');
    if (!confirmed) return;
      ...
      {!editing ? (
        <>
          <InfoGrid>
            ...
            <InfoItem><span>Data de nascimento</span><strong>{profile.birth_date || 'Não informado'}</strong></InfoItem>
          ...
          <DangerZone>
            <div>
              ...
              <span>Remove o perfil do MVP. Dados operacionais vinculados são apagados ou desvinculados para preservar a consistência do sistema.</span>
            </div>
            <DangerButton type="button" onClick={handleDeleteAccount} disabled={deleting}>
              {deleting ? <Spinner size="18px" /> : 'Excluir conta'}
            </DangerButton>
          </DangerZone>
        </>
      ) : (
