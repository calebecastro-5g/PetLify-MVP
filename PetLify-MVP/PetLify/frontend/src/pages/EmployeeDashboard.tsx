...
function EmployeeDashboard() {
  ...
  async function loadData() {
    ...
    setError('');
    try {
      const [appointmentsResponse, petsResponse, vaccinesResponse, teamResponse] = await Promise.all([
        apiFetch('/api/appointments'),
        apiFetch('/api/pets'),
        apiFetch('/api/vaccine-records'),
        apiFetch('/api/employees'),
      ]);
      if (!appointmentsResponse.ok || !petsResponse.ok || !vaccinesResponse.ok || !teamResponse.ok) {
        throw new Error('Falha ao carregar dados.');
      }
      const petsData = (await petsResponse.json()) as Pet[];
      const teamData = (await teamResponse.json()) as EmployeeOption[];
      ...
      if (!vaccineForm.veterinarian && teamData[0]) {
        setVaccineForm((prev) => ({ ...prev, veterinarian: teamData[0].name }));
      }
    } catch {
        ...
        <>
          ...
          <Section id="vacinas">
            ...
            <FormCard onSubmit={handleSaveVaccine}>
              ...
              <Field><span>Responsável</span><select value={vaccineForm.veterinarian} onChange={(event) => setVaccineForm((prev) => ({ ...prev, veterinarian: event.target.value }))}>{team.map((member) => <option key={member.id} value={member.name}>{member.name}</option>)}</select></Field>
